...
STREAM_MANAGER_URL = os.getenv('STREAM_MANAGER_URL', 'http://scorecastarr-stream:3001')
...
DB_PATH         = os.getenv('DB_PATH', '/config/scorecastarr.db')
    ...
    if _os.path.isdir(BUILTIN_AUDIO_SRC):
        ...
        if existing == 0:
            conn.execute("""INSERT INTO scoreboards(name,slug,is_default,sport_config,team_config,display_config)
                VALUES('ScorecastArr','scorecastarr',1,'{}','[]','{}')""")
        conn.commit()
...
def create_channels():
    ...
    if mode in ('combined','both'):
        make_channel('ScorecastArr — All Sports',ch_num,f'{STREAM_BASE_URL}/hls/scorecastarr.m3u8' if STREAM_BASE_URL else None)
        ch_num+=1
    if mode in ('per_sport','both'):
        for sport in sports:
            ...
            make_channel(f'ScorecastArr — {sname}',num,f'{STREAM_BASE_URL}/hls/{slug}.m3u8' if STREAM_BASE_URL else None,gid=gid)
            ch_num+=1
...
def backup_export():
    ...
    return Response(_json.dumps(payload,indent=2), mimetype='application/json',
        headers={'Content-Disposition':'attachment; filename=scorecastarr-backup.json'})
...
if __name__ == '__main__':
            ...
            def _parse_vehicles(vehicles):
                ...
                return [{
                    ...
                    'driver': ((v.get('driver', {}).get('full_name') if isinstance(v.get('driver'), dict) else None) or
                               ...
                               v.get('driver_name') or '?').rstrip(' #0123456789').strip(),
                    ...
                    if rs.ok:
                        ...
                        standings = [{
                            ...
                            'driver': ((e.get('driver', {}).get('full_name') if isinstance(e.get('driver'), dict) else None) or
                                       ...
                                       e.get('driver_name') or 'Unknown').rstrip(' #0123456789').strip(),
    ...
    log.info('ScorecastArr API starting on :5000')
