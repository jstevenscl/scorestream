...
- [Volumes](#volumes)
- [Using ScorecastArr](#using-scorecastarr)
...
curl -O https://raw.githubusercontent.com/jstevenscl/scorecastarr/main/docker-compose.yml
```
...
# Port ScorecastArr's web UI and HLS streams are exposed on
WEB_PORT=7777
...
| `SCORECASTARR_TAG` | `latest` | Image tag: `latest`, `beta`, or a pinned version like `v1.0.0` |
...
ScorecastArr encodes the rendered scoreboard to HLS via Chromium screenshots + x264. Three quality tiers are available via `STREAM_QUALITY`:
...
`STREAM_BASE_URL` is the address Dispatcharr will use to reach ScorecastArr's HLS streams. The correct value depends on how your stacks are arranged.

---

**Option A — ScorecastArr and Dispatcharr in the same Docker stack / network**

They share a Docker network and can reach each other by container name. Use the ScorecastArr web container name and its internal port:
...
**Option B — ScorecastArr and Dispatcharr in different stacks on the same host**
...
To find the internal Docker IP of the ScorecastArr web container (alternative to LAN IP):

```bash
docker inspect scorecastarr-web | grep '"IPAddress"'
```
...
**Option C — ScorecastArr and Dispatcharr on different hosts / VMs**

Use the LAN IP (or hostname) of the machine running ScorecastArr, with the exposed `WEB_PORT`:
...
ScorecastArr uses named Docker volumes. The first three are created automatically on first start.

| Volume | Contents |
|---|---|
| `scorecastarr_config` | SQLite database (`scorecastarr.db`) — stores all scoreboard settings |
| `scorecastarr_hls` | HLS stream segments — shared between the stream container and nginx |
| `scorecastarr_audio` | Uploaded audio files for background music |
| `scorecastarr_ticker` | Per-channel ticker text files shared with Dispatcharr (`scores_{channel_id}.txt`) — **required only for Ticker Overlay** (see [Ticker Overlay](#ticker-overlay)) |

> The `scorecastarr_ticker` volume is **not created automatically**. You must create it manually and add it to both stacks before using the Ticker Overlay feature. See [Shared Volume Setup](#shared-volume-setup).

**To back up your configuration:**

```bash
docker run --rm -v scorecastarr_config:/data -v $(pwd):/backup alpine \
  tar czf /backup/scorecastarr-config-backup.tar.gz -C /data .
```
...
```bash
docker run --rm -v scorecastarr_config:/data -v $(pwd):/backup alpine \
  tar xzf /backup/scorecastarr-config-backup.tar.gz -C /data
```
...
4. Click **Save & Close** — ScorecastArr immediately begins generating an HLS stream for this scoreboard
...
ScorecastArr includes 5 built-in color themes for stream output. The default is **Dark Blue** (`#00d4ff` accent).
...
**System Theme** controls the appearance of the ScorecastArr **interface itself** — the settings pages, sidebar, navigation buttons, section headers, and all UI controls. This is completely separate from stream themes.
...
> **Validation:** The importer rejects anything that isn't a valid ScorecastArr scoreboard export (`scorecastarr_export_version: 1, type: "scoreboard"`). Invalid JSON or missing fields show a clear error.
...
ScorecastArr can mix background music into the HLS stream output.
...
3. ScorecastArr creates or updates a channel in Dispatcharr with:
   - The scoreboard's name as the channel name
   - The HLS stream URL pointing to this server
   - The channel group "ScorecastArr"
...
2. Click **Push to Dispatcharr** again — ScorecastArr will **update** the existing channel (it does not create a duplicate)
...
4. When you click **Disable Ticker**, ScorecastArr restores the original profile ID and deletes the ticker copy
...
> **The original stream profile is never modified.** ScorecastArr creates a copy and assigns it. On disable, the original is restored cleanly.
...
> **FFmpeg parameter conflict checking:** When a channel's original stream profile contains parameters that are incompatible with the ticker overlay (such as `-force_key_frames`, hardware encoder filters, or conflicting scale settings), ScorecastArr automatically strips or replaces the conflicting parameters and records what was changed. You can see exactly what was adjusted in the Active Ticker Status Panel — each active ticker shows a collapsible **⚙ WARN/INFO** pill listing every modification and why it was made.
...
The per-channel ticker text files (`/ticker/scores_{channel_id}.txt`) must be accessible to both the ScorecastArr stream container (writer) and Dispatcharr's ffmpeg process (reader). This requires a shared Docker named volume.
...
Or in Portainer: **Volumes → Add volume**, name it `scorecastarr_ticker`, leave everything else default, click **Create**.

---

**Step 2 — Add the volume to your ScorecastArr stack**

In your ScorecastArr `docker-compose.yml`, add the volume mount to the `scorecastarr-stream` service and declare it at the bottom:
...
services:
  scorecastarr-stream:
    # ... existing config ...
    volumes:
      - scorecastarr_ticker:/ticker    # add this line
...
Navigate to **Ticker Overlay** in the ScorecastArr settings sidebar.
...
- A **⚙ WARN/INFO** pill — click to expand a panel showing every FFmpeg parameter that was adjusted when the ticker was enabled and why (e.g., `force_key_frames stripped — incompatible with encode mode`). This panel is purely informational; all adjustments are handled automatically by ScorecastArr.
...
ScorecastArr fetches the selected channel's current stream profile from Dispatcharr and validates it:
...
- **ENABLE TICKER** — saves config, creates the ticker stream profile in Dispatcharr, and assigns it to the channel. ScorecastArr begins writing the per-channel score file immediately. After enabling, the form clears (sport selections, custom text, CPU Saver checkboxes, channel search) so you can immediately configure a second ticker for a different channel.
...
The ticker overlay requires FFmpeg to decode and re-encode the video stream in real time. ScorecastArr uses `-c:v libx264 -preset ultrafast -tune zerolatency` for minimum latency and CPU overhead. For most setups this works well with CPU-only transcoding. If you experience stuttering on a lower-powered server (Oracle A1 Ampere or similar), enable one or more **CPU Saver** toggles — Cap at 15fps alone typically eliminates stuttering with no visible quality difference for a score ticker. GPU hardware encoding (NVENC, VAAPI, or QSV) is not required but will further reduce CPU load if available.
...
ScorecastArr organizes stream cards into **pages** — full-canvas views that rotate on the configured timer. Each page is filled dynamically based on the available resolution and card sizes.
...
Team logos sourced from ESPN are PNG images with transparent backgrounds, designed to display on white/light backgrounds. ScorecastArr renders them on a **white box with a team-color border** so every logo is clearly legible against the dark stream background — including teams whose primary color closely matches their own logo (e.g. Yankees navy, Cardinals red).
...
ScorecastArr displays player headshots on individual-sport cards (PGA, F1, NASCAR, ATP, WTA) when the **Player Headshots** toggle is enabled in the scoreboard editor's display settings.
...
- Ensure the Dispatcharr URL is reachable from the ScorecastArr container (use LAN IP, not localhost)
...
- Restart the `scorecastarr-stream` container after changing quality env vars: `docker restart scorecastarr-stream`
...
- **[Report a Bug](https://github.com/jstevenscl/scorecastarr/issues/new?template=bug_report.yml)** — something isn't working correctly
- **[Request a Feature](https://github.com/jstevenscl/scorecastarr/issues/new?template=feature_request.yml)** — suggest an improvement or new capability
